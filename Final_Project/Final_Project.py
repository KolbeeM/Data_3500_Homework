...
def get_dates():
    ...
    for i in range(3):
        ...
        dates.append({
            "url_date":     day.strftime("%Y%m%d"),      # ESPN needs no hyphens
            "display_date": day.strftime("%Y-%m-%d")     # CSVs look nicer with hyphens
        })
    return dates


def fetch_scoreboard(url_date):
    # hit the ESPN API for a specific date and return the JSON
    complete_url = url1 + url_date
    print(complete_url)
...
def parse_games(scoreboard, display_date, all_teams):
    # loop through each game and pull out the info we need
    games = []
...
def save_to_csv(games, all_teams, display_date):
    ...
    # append each game as a new row
    for g in games:
        games_file.write(g["date"] + ", " + g["game"] + ", " + g["status"] + ", " + g["home"]["team"] + ", " + g["home"]["score"] + ", " + g["away"]["team"] + ", " + g["away"]["score"] + "\n")
...
def analyze(all_games, all_teams):
    ...
    return top5, len(winning_teams), avg_score
...
for date in dates:
    scoreboard = fetch_scoreboard(date["url_date"])
    games      = parse_games(scoreboard, date["display_date"], all_teams)
    all_games += games
    save_to_csv(games, all_teams, date["display_date"])

top5, teams_above_500, avg_score = analyze(all_games, all_teams)

results = {
    "top_5_by_win_pct":   top5,
    "teams_above_500":    teams_above_500,
...
# load it back and print a quick summary to the console
loaded = json.load(open(RESULTS_FILE, "r"))
